// TM2025_WirelessLink (R 1.0):  This program implements a wireless link
// from a computer, via a USB serial port, to the TM2025_Logger, whch
// logs data from a TriMetric TM-2025 battery monitor (Bogart Engineering).
// It supports wireless access to the TM2025_Logger for configuration/control
// and downloading of the logged data.
// It configure some values in EEPROM for automatic configuration of the RF12 
// upon powerup.
...
static char cmd;
static byte value, stack[RF12_MAXDATA], top, sendLen, dest, quiet, verbose;
...
static void df_replay (word seqnum, long asof) {
    ...
    while (page != dfLastPage) {
        ...
        while (i < sizeof dfBuf.data && dfBuf.data[i] < 255) {
            ...
            Sprint(dfBuf.seqnum);
            Sprint(' ');
            Sprint(dfBuf.timestamp + dfBuf.data[i++]);
            Sprint(' ');
            Sprint((int) dfBuf.data[i++]);
            byte n = dfBuf.data[i++];
            while (n-- > 0) {
                Sprint(' ');
                Sprint((int) dfBuf.data[i++]);
...
char helpText1[] PROGMEM = 
    ...
    "  <n> v      - set verbose mode (1 = output details of processing)" "\n"
    ...
    if ('0' <= c && c <= '9')
       ...
       if (1 == verbose) {
        ...
        switch (c) {
            ...
            case 's': // send packet to node ID N, no ack request
                cmd = c;
                sendLen = top;
                dest = value;
            ...
            case 'w': // wipe entire flash memory
                if (df_present() && stack[0] == 12 && value == 34)
                    df_wipe();
                break;
            case 'q': // turn quiet mode on or off (1 = don't report bad packets)
                quiet = value;
                break;
            case 'v': // turn verbose mode on or off (1 = print processing details)
                verbose = value;
                break;
            }
        value = top = 0;
...
void setup() {
    ...
    if (rf12_config(0)) {
        ...
        config.nodeId = 0x41; // node A(1) @ 433 MHz
...
void loop() {
    ...
    if (rf12_recvDone()) {
        byte len = rf12_len;
        if ((rf12_crc == 0) && LastTime) {
           CurrentTime = millis();
           DeltaTime = CurrentTime - LastTime;
           LastTime = 0;
           if (1 == verbose) {
              Sprint ("Time Delta = ");
              Sprint (DeltaTime, DEC);
              Sprintln ("ms");
              }
           }
        if (rf12_crc == 0) {
           ...
           if (RF12_WANTS_ACK && (config.nodeId & COLLECT) == 0) {
              if (1 == verbose) 
                 Sprintln (" sending ack");
              byte Header = RF12_HDR_CTL;
              Header |= config.nodeId & 0x1F;
              rf12_sendStart (Header, 0, 0);
              }
           activityLed (0);
           }
        else if (quiet)
           ...
           if (1 == verbose) 
              Sprint ("CRC BAD");
           if (len > 20)           // print at most 20 bytes if crc is wrong
              len = 20;
           }
        if (config.group == 0) {
           if (1 == verbose) {
        ...
        if (1 == verbose) {
           Sprint (" HDR=");
           Sprint ((int) rf12_hdr, HEX);
           Sprint ("h");
           Sprint (" LEN=");
           Sprint ((int) len, DEC);
           }
        // dump uplaoded packet to serial port
        if (0 < len) {
           Sprint (" (");
           for (byte i = 0; i < len; ++i) {
               if (i != 0)
                    Sprint (' ');
               Sprint ((unsigned char) rf12_data[i], HEX);
               }
           Sprint(")");
           }
        if (1 == verbose) 
           if (rf12_hdr & RF12_HDR_CTL) {
              if (dest == (rf12_hdr & RF12_HDR_MASK)) 
                 Sprint (" Ack rcvd for sent packet from ");
              else
                 Sprint (" Ack rcvd for unsent packet from ");
              Sprint ((int) (rf12_hdr & RF12_HDR_MASK));
              }
        Sprintln();
        }
    ...
    if (cmd && rf12_canSend()) {
        ...
        if (1 == verbose) {
           ...
           for (byte i = 0; i < sendLen; ++i) {
              if (i != 0)
                 Sprint (' ');
              Sprint ((unsigned char) testbuf[i], HEX);
              }
           Sprint (") to ");
           Sprint ((int) dest);
           }
