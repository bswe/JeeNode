...
#define P2D_PIN     5          // port 2 pin DIO
#define P2A_PIN     15         // port 2 pin AIO
#define GREEN_LED   P2D_PIN    // red activity LED 
#define RED_LED     P2A_PIN    // green activity LED 
#define ON          1
#define OFF         0
...
#define PACKET_INTERVAL_TIME 2000
#define RESPONSE_TIMEOUT_DURATION 1000

boolean manual = true;
unsigned long ResponseTimeoutTime = 0;
unsigned long SendNextPacketTime;
...
static void activityLed (byte pin, byte on) {
    pinMode(pin, OUTPUT);
    digitalWrite(pin, !on);
}
...
char helpText1[] PROGMEM = 
    ...
    "    push red LED button to toggle between auto/manual\n"
    "    push green LED button to send packet in manual mode\n\n"
    "Available commands:" "\n"
...
void loop() {
    ...
    ButtonState = port2_blinkPlug.buttonCheck();
    if (ButtonState == BlinkPlug::ON2) {
       //Serial.println("button 2 pressed");
       do 
          ButtonState = port2_blinkPlug.buttonCheck();
       while (ButtonState != BlinkPlug::OFF2);
       ...
       if (manual)
          ...
          SendNextPacketTime = millis() + PACKET_INTERVAL_TIME;
          }
       }

    if (ResponseTimeoutTime) {    // waiting for response so don't send another packet
       if (millis() > ResponseTimeoutTime) {
          // timed out waiting for response so flash red LED
          activityLed(RED_LED, ON);
          delay (BLINK_DURATION);
          activityLed(RED_LED, OFF);
          Serial.println("response timed out");
          // reset response timeout to indicate not waiting for ack
          ResponseTimeoutTime = 0;
          }
       }
    else if (manual) {
       if (ButtonState == BlinkPlug::ON1) {
          //Serial.println("button 1 pressed");
          do 
             ButtonState = port2_blinkPlug.buttonCheck();
          while (ButtonState != BlinkPlug::OFF1);
          //Serial.println("button 1 releassed");
          // send a broadcast packet with ack request
          cmd = 'a';
          sendLen = RF12_MAXDATA;
          dest = 0;
          for (byte i = 0; i < RF12_MAXDATA; ++i)
    ...
    else if (millis() >= SendNextPacketTime) {
       SendNextPacketTime = millis() + PACKET_INTERVAL_TIME;
       cmd = 'a';
       sendLen = RF12_MAXDATA;
       dest = 0;
    ...
    if (rf12_recvDone()) {
        ...
        if (rf12_crc == 0) {
            Serial.print("OK");
            activityLed(GREEN_LED, ON);
            delay (BLINK_DURATION);
            activityLed(GREEN_LED, OFF);
        } else {
            if (quiet)
                return;
            activityLed(RED_LED, ON);
    ...
    if (cmd && rf12_canSend()) {
        ...
        ResponseTimeoutTime = millis() + RESPONSE_TIMEOUT_DURATION;
